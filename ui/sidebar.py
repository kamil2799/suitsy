...
def render_sidebar(username, portfolio):
    with st.sidebar:
        ...
        benchmarks = st.multiselect("Benchmarki:", ["S&P 500", "NASDAQ 100", "WIG20", "Złoto", "Bitcoin"],
                                    default=["S&P 500"]) if show_bench else []
                ...
                if st.form_submit_button("Dodaj"):
                    ...
                    if err:
                        ...
                        with st.spinner("Przeliczam..."):
                            ...
                            if hist.empty:
                                st.error("Brak ceny dla tej daty!")
                            else:
                                ...
                                portfolio.append(
                                    {'Symbol': symbol, 'Data_Zakupu': d_in.strftime('%Y-%m-%d'), 'Waluta': asset_curr,
                                     'Ilosc': qty, 'Kwota_Poczatkowa_PLN': cost_pln, 'Notatka': n_in})
                                if save_user_data(username, portfolio): st.success("Dodano!"); st.rerun()

        with st.expander("Edytuj Notatkę"):
            if portfolio:
                idx = st.selectbox("Transakcja",
        ...
        with st.expander("Usuń pozycję"):
            if portfolio:
                idx_del = st.selectbox("Wybierz do usunięcia",
                                       [f"{i + 1}. {p['Symbol']} ({p['Data_Zakupu']})" for i, p in
                                        enumerate(portfolio)])
